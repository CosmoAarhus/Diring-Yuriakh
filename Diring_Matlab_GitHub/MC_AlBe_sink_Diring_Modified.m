...
%
% The workflow of the program is:t 
...
load mc_source_Diring_Modified.mat                               % Load file with sink simulations
name = 'mc_sink_Diring_Modified';                                % Filename for saving
tic 
...
lon = 128.47;                                           % Longitude of sink site
elev = 215;                                             % Elevation of sampling site [m asl.]

%*********** Sink history ************
min_sink_time = 0.0;                                      % Possible burial times in sink [Myr]         
max_sink_time = 2.0;                                      % Possible burial times in sink [Myr]
min_rate = 1000;                                          % Possible erosion/accumulation rates in sink [g/cm2/Myr] (Depending on ero setting)
max_rate = 12000;                                         % Possible erosion/accumulation rates in sink [g/cm2/Myr]

depth = 400;                    % IF ACCUMULATION: Input estimated depth of samples within unit [g/cm2]
...
%%% Parameters for testing post-burial re-exposure after the LGM
LGM_Expo_Dur = 0; %Duration of post-LGM re-exposure (in years) 
LGM_Expo_Time = 15e3; %Timing of post-LGM re-exposure (in years) 
LGM_thick_Old = 0; %Thickness in cm  of "old" sediment cover (Layers 6-13m Units IIIa-IIIe)
LGM_thick_Young = 250; %Thickness in cm of "young" sediment cover (Layers 14-16, Unit IV)
rho_sed = 2.0; %Density of eolian sediment overlying the stone artifacts


%%% END - Parameters for testing post-burial re-exposure after the LGM
...
t_expo2 = randi([0.9*LGM_Expo_Dur 1.1*LGM_Expo_Dur],Nsim,1);
t_LGM = LGM_Expo_Time;
D_LGM = LGM_thick_Old; %Thickness in cm  of "old" sediment cover (Layers 6-13)
D_postLGM = LGM_thick_Young; %Thickness in cm of "young" sediment cover (Layers 14-16)
P10_DY = 5.0167*exp(-D_LGM*rho_sed/160);
P26_DY = P10_DY*6.95;

P10_DY_postLGM = 5.0167*exp(-D_postLGM*rho_sed/160);
P26_DY_postLGM = P10_DY_postLGM*6.95;
...
parpool;
    ...
    parfor (j=1:Nsim,3)
        ...
        NBe_tot(j) = (NBe_inh_run(1) + NBe_post_run(1) + (P10_DY/(lambda_Be))*(1-exp(-lambda_Be*t_expo2(j))))*exp(-lambda_Be*t_LGM) + (P10_DY_postLGM/(lambda_Be))*(1-exp(-lambda_Be*t_LGM));
        NAl_tot(j) = (NAl_inh_run(1) + NAl_post_run(1) + (P26_DY/(lambda_Al))*(1-exp(-lambda_Al*t_expo2(j))))*exp(-lambda_Al*t_LGM) + (P26_DY_postLGM/(lambda_Al))*(1-exp(-lambda_Al*t_LGM));
...
%         NAl_tot(j) = (NAl_inh_run(1) + NAl_post_run(1)); 
    ...
    t_expo = mc_source.t_expo';
    t_bur = mc_source.t_bur';

    rate = rate_fin;

    save(name,'NBe_inh_initial','NAl_inh_initial','NBe_post',...
        'NAl_post','NBe_tot','NAl_tot','rate','Ages_fin','storage_time','storage_depth',...
        'source_pluck','Tice','t_expo','t_expo2','t_bur','erate_source','Nt','Ne','elevation_source','ero','-v7.3')
