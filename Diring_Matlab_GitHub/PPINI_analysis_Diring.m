...
site = ' ';
name = 'mc_sink_Diring';    % Library name 
...
%     set(propert,'FontSize',15)
    ...
    hist_info{j} = [Ages_fin(filter);erate_fin(filter)*1e6;erate_source(filter)*1e6;NBe_inh(filter);NAl_inh(filter)];
...
mean_NBe_inh(j) = mean(NBe_inh);
unc_NBe_inh(j) = std(NBe_inh);
