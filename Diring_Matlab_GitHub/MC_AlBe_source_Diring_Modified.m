...
name = 'mc_source_Diring_Modified.mat';                                 % Filename
Nsim = 10e6;                                                  % Number of simulations

%*********** Geographical settings ************
lat = 61.2;                                                  % Latitude of sink site
lon = 128.47;                                                % Longitude of sink site, negative if W
sink_elev = 215;                                             % Elevation of sampling site [m asl.]
max_elev = 1000;                                             % Maximum elevation of the possible source area [m asl.]

%************* Landscape settings *************
rho_min = 2.5;                                               % Minimum rock density in source [g/cm3]
rho_max = 2.7;                                               % Maximum rock density in source [g/cm3]
min_ero = 1;                                                % Minimum source interglacial erosion rate [m/Myr]
max_ero = 1000;                                              % Miximum source interglacial erosion rate [m/Myr]

%************* Advanced settings **************
egla = 0e-6;                                                 % Glacial erosion rate [m/yr]
pluck_min = 0.01;                                            % Minimum plucking depth pr. glaciation [m] 
pluck_max = 5;                                              % Maximum plucking depth pr. glaciation [m] 
ice_lengths = 2000 ;                                         % Maximum potential length of a glaction at site [yr]
n_ice_max = 1;                                               % Number of glaciations possible (from 1 up to x)

P_SLHL_Be = 4;                                               % 10Be production sea-level high-latitude [at/g/yr]
R_SLHL = 6.75;                                                % 26Al/10Be production ratio at SLHL
P_SLHL_Al = R_SLHL*P_SLHL_Be;                                % 26Al production SLHL
% R_SLHL =  round(680 + (697-680) .* rand(Nsim,1))/100; 
dt = 2000;                                                   % Timesteps in calculations [yr]

%%% Modified source parameters
t_bur = randi([1e3 5e6],Nsim,1);
t_expo = randi([1e1 50e3],Nsim,1);
P10_DY = 5.0167;
P26_DY = P10_DY*6.95;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%           Code form here           %%%%%%%%%%%%%%%%%%%%
...
parfor (j=1:Nsim,3)
    ...
    for kk=(ntime-1):-1:1 % Go through all the burial timesteps from oldset to now.
          ...
          Pz_Be = pfac(kk)*(Pspal_Be*exp(-rho*100*burial(kk)/Lspal_Be)...           % Balco
                ...
                +a2_Be*exp(e2_Be*rho*100*burial(kk))...j
    ...
    NBe_NoExpo(j) = NBe(1); % Newest concentrations
    NAl_NoExpo(j) = NAl(1);


    NBe_exp(j) = NBe(1)*exp(-lambda_Be*(t_bur(j)+t_expo(j)))+(P10_DY/(lambda_Be))*(1-exp(-lambda_Be*t_expo(j))); % Newest concentrations
    NAl_exp(j) = NAl(1)*exp(-lambda_Al*(t_bur(j)+t_expo(j)))+(P26_DY/(lambda_Al))*(1-exp(-lambda_Al*t_expo(j))); % Newest concentrations;
%     NBe_exp(j) = NBe_NoExpo(j);
%     NAl_exp(j) = NAl_NoExpo(j);
    Pspal(j) = Pspal_Be;
    NBe_source(j) = NBe_exp(j); 
    NAl_source(j) = NAl_exp(j); 
    erate_source(j) = eint;
...
k_sort = find(1e8 > NAl_exp & NAl_exp > 0);
NBe_exp_sort = NBe_exp(k_sort);
NAl_exp_sort = NAl_exp(k_sort);
Pspal_sort = Pspal(k_sort);
NBe_source_sort = NBe_source(k_sort);
NAl_source_sort = NAl_source(k_sort);
erate_source_sort = erate_source(k_sort);
max_burial_sort = max_burial(k_sort);
Tice_sort = Tice(k_sort);
NBe_NoExpo_sort = NBe_NoExpo(k_sort);
NAl_NoExpo_sort = NAl_NoExpo(k_sort);
t_expo_sort = t_expo(k_sort);
t_bur_sort = t_bur(k_sort);
source_pluck_sort = source_pluck(k_sort);
elevation_sort = elevation(k_sort);
Nsim = length(t_expo_sort);

k_sort2 = find(NAl_exp_sort./NBe_exp_sort < 8.5 & NAl_exp_sort./NBe_exp_sort > 1);
NBe_exp_sort2 = NBe_exp_sort(k_sort2);
NAl_exp_sort2 = NAl_exp_sort(k_sort2);
Pspal_sort2 = Pspal_sort(k_sort2);
NBe_source_sort2 = NBe_source_sort(k_sort2);
NAl_source_sort2 = NAl_source_sort(k_sort2);
erate_source_sort2 = erate_source_sort(k_sort2);
max_burial_sort2 = max_burial_sort(k_sort2);
Tice_sort2 = Tice_sort(k_sort2);
NBe_NoExpo_sort2 = NBe_NoExpo_sort(k_sort2);
NAl_NoExpo_sort2 = NAl_NoExpo_sort(k_sort2);
t_expo_sort2 = t_expo_sort(k_sort2);
t_bur_sort2 = t_bur_sort(k_sort2);
source_pluck_sort2 = source_pluck_sort(k_sort2);
elevation_sort2 = elevation_sort(k_sort2);
Nsim = length(t_expo_sort2);


NBe_fin = NBe_exp_sort2;
NAl_fin = NAl_exp_sort2;

figure;
hold on
plot(NBe_fin,NAl_fin,'k.');
...
mc_source.source_pluck = source_pluck_sort2;
mc_source.Tice = Tice_sort2;
mc_source.P_source = Pspal_sort2;
mc_source.NBe_source = NBe_source_sort2;
mc_source.NAl_source = NAl_source_sort2;
mc_source.erate_source = erate_source_sort2;
mc_source.elevation_source = elevation_sort2;
mc_source.max_burial = max_burial_sort2;
mc_source.t_expo = t_expo_sort2;
mc_source.t_bur = t_bur_sort2;
